...
def _get_local_from_bucket_fpaths(base_dir, satellite, bucket_fpaths):
    ...
    fpaths = [os.path.join(base_dir, satellite, _remove_bucket_address(fpath)) for fpath in bucket_fpaths]
    return fpaths
...
def download_files(
    ...
    for start_time, end_time in time_blocks:
        ...
        _ = remove_corrupted_files(local_fpaths=local_fpaths, bucket_fpaths=bucket_fpaths, fs=fs)
    ...
    if verbose:
        ...
        print(f"--> {n_downloaded_files} files have been downloaded in {t_elapsed} seconds !")
        print("-------------------------------------------------------------------- ")
...
def download_closest_files(
    ...
    _check_download_protocol(protocol)
    # Get closest time
    closest_start_time, closest_end_time = find_closest_start_time(
        time=time,
        base_dir=None,
        protocol=protocol,
        fs_args=fs_args,
